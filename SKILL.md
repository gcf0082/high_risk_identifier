---
name: high-risk-scan
description: 对给定目录做疑似高风险点识别与审计。先跑规则扫描脚本(scan.py + rules.yaml),再按优先级逐条审核命中是真是假(例如命令执行命中要确认是否真的会执行命令),然后基于对项目的理解动态补充一份高优先级、低误报的自定义规则做第二轮扫描,最后整合为带确认状态的增强 JSON 报告。当用户要求"扫描/审计/识别某个目录或项目的高风险点、危险操作、命令执行、敏感文件"时,或者提到网管产品(NCE/MAE)代码安全排查时,务必使用本 skill,即使用户没有明说"高风险扫描"。
---
